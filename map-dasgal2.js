...
console.log(getLargeEngineCars(cars));
...
function getUniqueBrands(cars) {
  ...
  carbrands.forEach((car) => {
    uniqueBrands[car] = true;
  });
  return Object.keys(uniqueBrands);
}
console.log("17", getUniqueBrands(cars));

// 18. Зөвхөн model ба year тайлбартай шинэ массив үүсгэдэг функц бич.
function getModelAndYearList(cars) {
  return cars.map((car) => {
    return { model: car.model, year: car.year };
  });
}
console.log("18", getModelAndYearList(cars));
...
console.log("19", tagSuperEfficientCars(cars));
...
console.log("20", addIdToCars(cars));
